...
struct HomeView: View {
    ...
    private var consumerHomeView: some View {
        ...
        NavigationStack {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    if let last {
                        LastReceiptCard(receipt: last,
                                        ...
                                        onShare: {
                                            // 👇 [수정된 부분] NSDecalNumber -> NSDecimalNumber 로 오타 수정
            ...
            // .navigationTitle("OURECEIPT 🌳") // 이 줄은 삭제합니다.
            .toolbar {
                // 👇 [추가된 부분] 히든 버튼을 위한 코드
                ToolbarItem(placement: .principal) {
                    Text("OURECEIPT 🌳")
                        .font(.headline)
                        .onTapGesture(count: 5) { // 5번 탭하면 동작
                ...
                // ---
                ToolbarItem(placement: .topBarLeading) { Button { } label: { Image(systemName: "line.3.horizontal") } }
